    ...
    <div class="musicList">
        ...
        <div class="listContent">
            ...
            <van-swipe :loop="false" :width="150" :height="150" :show-indicators="false">
                <van-swipe-item v-for="m in musicList" :key="m.id" class="listItem">
                    <!-- 实现了路由跳转并带参数 -->
                    <router-link :to="{
                        path:'/MusicItem', 
                        query:{id:m.id}
                        ...
                        <span class="tip">
                            ...
                            {{ numeral(m.playCount).format('0.0a') }}
                        </span>
    ...
    export default {
        ...
        setup() {
            ...
            onMounted(async () => {//成功用async 和await完成
                ...
                //     }
                // )
                let res = (await getMusicList()).data.result
                musicList.push(...res)
                console.log(musicList);
            })
            return {musicList,numeral}
        }
    }
</script>
